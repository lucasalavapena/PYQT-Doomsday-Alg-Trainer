...
def rand_date(earliest_date, latest_date):
    differenceDateSeconds = (latest_date - earliest_date).total_seconds()
    date = earliest_date + datetime.timedelta(seconds=random.randint(0, int(differenceDateSeconds)))
    return date
...
def worked_date(date):
    ...
    help_text += """2. The anchor day is {0} as the date is in the {1}s. This can either be memorized
     or calculated using: 5 * (century % 2) % 7 + 2.\n \t In this case: 5 * ({2} % 2) % 7 + 2 = {0}
     \n""".format(anchor_day, anchorday_key, int(anchorday_key/100))
    ...
    leap year we need to use ddalg_step2 different set of doomsday for that year so lets check that first."""
    if is_leap_year(date.year) and date.month < 3:
        closet_doomsday = DoomsdayNonLeap.get(date.month)[0]
